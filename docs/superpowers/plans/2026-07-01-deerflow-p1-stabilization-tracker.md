...
| P1-B 任务详情视觉补证 | 进行中 | Task detail screenshots/API summaries cover layout, execution steps, inline thinking, Mermaid, Artifacts, export, token modes, memory entry, loading/error states, and narrow/mobile risk cases. | 2026-07-02 `TD-LAYOUT-002` is closed: task-detail now scopes narrow responsive overrides to the mounted page, 768px/390px browser evidence shows `horizontalOverflow=false`, and screenshots are recorded in `screenshots/coze/P1-B-TD-LAYOUT-002-coze-768-task-detail.png` and `screenshots/coze/P1-B-TD-LAYOUT-002-coze-390-task-detail.png`. Existing desktop evidence covers most modules. Remaining P1-B items are smaller product decisions or edge evidence: assistant feedback affordance, real invalid Mermaid screenshot, subagent deep card, upload path, and active streaming token state. |
...
Continue P1-B only if the remaining smaller visual decisions are still launch
blocking:

- Decide whether `TD-MSG-006`, `TD-MD-003`, `TD-FLOW-003`, `TD-COMP-006`, and
  `TD-TOKEN-003` stay in P1 or move to P2.
