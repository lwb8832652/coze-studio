...
func TestCheckReadinessMapsSDKErrorsAndRespectsContextCancel(t *testing.T) {
	sdkErr := errors.New("sdk unavailable")
	clients := []struct {
		name    string
		new     func(context.Context, domain.PublicConfig, domain.CredentialInput) (storage.Storage, error)
		cfg     domain.PublicConfig
		install func() func()
	}{
		{
			name: "minio",
			new:  miniopkg.NewFromConfig,
			cfg:  domain.PublicConfig{Bucket: "coze", Endpoint: "127.0.0.1:1", UseSSL: false},
			install: func() func() {
				return miniopkg.SetReadinessCheckForTest(func(context.Context, string) (bool, error) {
					return false, sdkErr
				})
			},
		},
		{
			name: "s3",
			new:  s3pkg.NewFromConfig,
			cfg:  domain.PublicConfig{Bucket: "coze", Region: "us-east-1", Endpoint: "http://127.0.0.1:1", ForcePathStyle: true},
			install: func() func() {
				return s3pkg.SetReadinessCheckForTest(func(context.Context, string) error {
					return sdkErr
				})
			},
		},
		{
			name: "tos",
			new:  tospkg.NewFromConfig,
			cfg:  domain.PublicConfig{Bucket: "coze", Region: "cn-beijing", Endpoint: "http://127.0.0.1:1"},
			install: func() func() {
				return tospkg.SetReadinessCheckForTest(func(context.Context, string) error {
					return sdkErr
				})
			},
		},
	...
	for _, tc := range clients {
		t.Run(tc.name, func(t *testing.T) {
			restore := tc.install()
			defer restore()
			client, err := tc.new(context.Background(), tc.cfg, domain.CredentialInput{AccessKeyID: "ak", SecretAccessKey: "sk"})
			if err != nil {
				t.Fatalf("NewFromConfig() error = %v", err)
			}
			readiness, ok := client.(storage.ReadinessChecker)
			if !ok {
				t.Fatal("NewFromConfig() result does not implement storage.ReadinessChecker")
			}

			canceled, cancel := context.WithCancel(context.Background())
			cancel()
			if err := readiness.CheckReadiness(canceled); !errors.Is(err, context.Canceled) {
				t.Fatalf("CheckReadiness(canceled) error = %v", err)
			}

			if err := readiness.CheckReadiness(context.Background()); !errors.Is(err, storage.ErrReadinessUnavailable) {
				t.Fatalf("CheckReadiness(unavailable) error = %v", err)
			}
		})
	}
}
