...
| P1-C-003 UI screenshots per MIME family | 进行中 | Browser screenshots recorded for full card placement, Markdown preview, TXT/JSON/CSV preview, PNG preview, and HTML/SVG download-only active-content safety. PDF right-side iframe preview is implemented and unit-tested, but browser screenshot is still pending because the in-app browser automation timed out while reloading after the frontend change. Remaining: PDF screenshot and copy/download affordance checks. Evidence: `notes/P1-C-artifact-mime-sample-map.md`; screenshots under `screenshots/coze/P1-C-coze-*-7657556998191316992.png`. |
...
- Capture the remaining PDF side-preview screenshot and copy/download affordance
  checks from fixture task `7657556998191316992`.
