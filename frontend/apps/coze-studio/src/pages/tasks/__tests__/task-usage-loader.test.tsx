...
const renderUsageHarness = (threadID: string, spaceID = 'space-1') => {
  ...
  act(() =>
    root.render(<UsageHarness spaceID={spaceID} threadID={threadID} />),
  );

  return {
    rerender: (nextThreadID: string) =>
      act(() =>
        root.render(<UsageHarness spaceID={spaceID} threadID={nextThreadID} />),
      ),
    rerenderScope: (nextThreadID: string, nextSpaceID: string) =>
      act(() =>
        root.render(
          <UsageHarness spaceID={nextSpaceID} threadID={nextThreadID} />,
        ),
      ),
  };
};
...
describe('useTaskUsageData scope', () => {
  ...
  it('invalidates usage data when the workspace changes for the same thread', async () => {
    ...
    const { rerenderScope } = renderUsageHarness('thread-shared', 'space-1');
