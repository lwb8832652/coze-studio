...
| 上线验收-桌面端任务冒烟 | 进行中 | Manual or automated browser path covers P0 task lifecycle. | Partial browser smoke passed for create, ADK execution, detail refresh, message rendering, and token usage on local `127.0.0.1:8888` with `AGENT_THREAD_RUNTIME_DEFAULT=eino_adk`, `AGENT_THREAD_EINO_ADK_ENABLED=true`, and `AGENT_THREAD_WORKER_ENABLED=true`. Remaining before release candidate: cancel/retry button path and task memory open/edit smoke. |
