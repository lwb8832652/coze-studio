...
| P1-C-003 UI screenshots per MIME family | 进行中 | Browser screenshots recorded for full card placement, Markdown preview, TXT/JSON/CSV preview, PNG preview, and HTML/SVG download-only active-content safety. Current fixture PDF is still `scan_pending`; safe blocked-state copy is covered by service/task-detail tests, and full browser lifecycle evidence remains under P1-C-005. PDF right-side iframe preview is implemented and unit-tested, but the clean browser screenshot remains pending because the fixture PDF has not been released by scanner/review flow. Remaining: clean PDF iframe screenshot and copy/download affordance checks. Evidence: `notes/P1-C-artifact-mime-sample-map.md`; screenshots under `screenshots/coze/P1-C-coze-*-7657556998191316992.png`. |
...
- Finish scan job/review-flow evidence, then capture the remaining clean PDF
  side-preview screenshot and copy/download affordance checks from fixture task
  `7657556998191316992` or an equivalent released PDF artifact.
