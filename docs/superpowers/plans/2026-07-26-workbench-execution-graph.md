...
- Modify `.gitignore`: ignore the complete generated `docs/superpowers/context/workbench-execution-graphify/` tree.
...
- [ ] **Step 1: Write failing shape, uniqueness, and reachability tests**
...
assert.match((await validateContract(withDuplicateNode, fixture)).errors.join('\n'), /duplicate_node_id/);
assert.match((await validateContract(withDanglingEdge, fixture)).errors.join('\n'), /edge_target_missing/);
assert.match((await validateContract(withBrokenOrder, fixture)).errors.join('\n'), /chain_edge_order_invalid/);
assert.match((await validateContract(withMissingEvidence, fixture)).errors.join('\n'), /chain_test_evidence_missing/);
```
...
- [ ] **Step 3: Implement deterministic contract validation**

Export these functions with no third-party dependencies:

```js
export const CURRENT_STATUS = 'current';
export const validateContract = async (contract, options) => ({ errors, warnings, resolvedVersions });
...
- [ ] **Step 1: Add a failing test that validates the real repository contract**
...
- [ ] **Step 3: Author the machine contract with exact stable IDs**
...
- [ ] **Step 4: Author the human-readable chain from the same facts**
...
- [ ] **Step 5: Run the real-contract test and confirm GREEN**
...
- [ ] **Step 1: Write failing framework and boundary tests**

Add fault-injection tests for these exact conditions:

```js
assert.match(errorsFor(versionDrift), /framework_version_mismatch/);
assert.match(errorsFor(canonicalWithoutProductionAnchor), /canonical_framework_anchor_missing/);
assert.match(errorsFor(swappedMiddleware), /middleware_order_mismatch/);
assert.match(errorsFor(langGraphAsRuntime), /noncanonical_runtime_in_execution_chain/);
...
```js
assert.deepEqual(evaluateAuthorityChanges(['backend/application/agentthread/adk_executor.go']), ['authority_files_not_updated']);
assert.deepEqual(evaluateAuthorityChanges([CONTRACT_RELATIVE]), ['authority_files_must_change_together']);
assert.deepEqual(evaluateAuthorityChanges([CONTRACT_RELATIVE, CONTEXT_RELATIVE]), []);
```
...
- [ ] **Step 3: Implement manifest-backed versions and boundary checks**
...
- [ ] **Step 5: Commit framework validation**
...
- [ ] **Step 1: Write failing deterministic corpus and merge tests**
...
assert.equal(findLink(graph, 'entry.workbench', 'api.create_thread').relation, 'calls');
assert.equal(findLink(graph, 'entry.workbench', 'api.create_thread').confidence, 'EXTRACTED');
```
...
- [ ] **Step 3: Implement deterministic corpus and digest generation**
...
export const writeCorpus = async (contract, options) => ({ digest, files, corpusDir });
export const sha256Files = async (files, root) => digest;
```

Write generated files only below `docs/superpowers/context/workbench-execution-graphify/`. Render `contract-ledger.md`, copy the committed authority Markdown, and copy every unique source/evidence/version file under `corpus/source/<repo-relative-path>`. Sort all paths and JSON keys before hashing.

- [ ] **Step 4: Implement Graphify AST build and explicit graph merge**
...
Then load `graphify-out/graph.json`, force `directed: true`, preserve AST nodes/links, inject contract nodes with `source_file` and `source_location`, inject explicit edges with `chain_ids`, and write `build-meta.json` containing schema version, Git commit, corpus digest, Graphify version, and generation timestamp. Use `spawn` argument arrays, never shell interpolation.
...
- [ ] **Step 5: Ignore all generated artifacts**
...
- [ ] **Step 1: Write failing CLI and derived fault-injection tests**

Cover `verify`, `build`, `verify-derived`, unknown commands, stale digest, dangling endpoints, duplicate relation edges, self-loops, missing required nodes, broken ordered paths, forbidden current nodes, and Graphify query smoke failures. CLI errors must be concise and must exit nonzero.
...
- [ ] **Step 4: Implement derived health and query verification**

Export:

```js
export const verifyDerivedGraph = async (contract, options) => ({ errors, warnings });
export const verifyRequiredQueries = (graph, requiredQueries) => [];
export const verifyOrderedPaths = (graph, chains) => [];
```

Check current digest, directed mode, endpoint validity, duplicate `(source,target,relation)` tuples, self-loops, required nodes/edges, chain order, framework scope, and exclusion absence. For each required query, verify exact IDs internally and run a Graphify CLI smoke query using only contract-provided `expanded_terms`; required labels must appear in output.

- [ ] **Step 5: Run all tests and real verification**
...
- [ ] **Step 1: Write a failing documentation-reference test**
...
- [ ] **Step 4: Add a short AGENTS.md entry**
...
- [ ] **Step 1: Run the complete test and static verification suite**
...
- [ ] **Step 2: Build and verify the actual directed graph**
...
- [ ] **Step 3: Run user-critical graph queries**
...
- [ ] **Step 4: Refresh and verify CodeGraph/codebase-memory**
...
- [ ] **Step 6: Commit any audit-only corrections and stop before merge**
