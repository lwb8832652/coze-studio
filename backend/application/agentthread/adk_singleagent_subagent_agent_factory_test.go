...
func TestADKSingleAgentSubagentAgentFactoryBuildsRunnableChildAgent(
	...
	require.Contains(t, childModel.messages[0].Content, adkLeadPromptContractVersion)
	require.Contains(t, childModel.messages[0].Content, prompt)
	require.NotContains(t, childModel.messages[0].Content, "<todo_system>")
	require.NotContains(t, childModel.messages[0].Content, "<subagent_system>")
...
func TestADKSingleAgentSubagentRunSummaryMapsStableSnapshotFields(
	...
	require.Equal(t, string(RuntimeModeEinoADK), payload["runtime"])
	require.Equal(t, string(DeerFlowRequestedPolicyPro), payload["requested_policy"])
	require.Equal(t, string(DeerFlowModePro), payload["mode"])
	require.Equal(t, false, payload["thinking_enabled"])
	require.Equal(t, false, payload["is_plan_mode"])
	require.Equal(t, false, payload["subagent_enabled"])
