...
| P1-C-003 UI screenshots per MIME family | 进行中 | Browser screenshots recorded for full card placement, Markdown preview, TXT/JSON/CSV preview, PNG preview, HTML/SVG download-only active-content safety, and PDF iframe mount after review release. Scan-pending safe blocked-state copy is covered by service/task-detail tests. Remaining: PDF page-pixel render capture and copy/download affordance checks. Evidence: `notes/P1-C-artifact-mime-sample-map.md`; screenshots under `screenshots/coze/P1-C-coze-*-7657556998191316992.png`. |
...
- Continue artifact lifecycle evidence from fixture task `7657556998191316992`:
  blocked review, retry, clean post-fix restore recapture, and copy/download
  affordance checks.
- Capture bounded API summaries for artifact list/content/signed URL and the
  scan/delete/restore lifecycle, without raw object URI, signed URL, provider
  payload, scanner body, prompt, completion, or checkpoint bytes.
