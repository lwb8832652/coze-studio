...
func TestCanonicalRunProjectionRedactsInternalFields(t *testing.T) {
	projected, err := projectCanonicalRun(&appagentthread.RunSummary{
		...
		Metadata:          `{"source":"workbench_home","source_run_id":2999,"appended_message_id":"4001","parent_run_id":2998,"requested_at":1785052799000,"status":"failed","business_key":"release-plan","business_id":9007199254740993,"user_id":"42","error_chain":"` + canonicalProjectionSensitiveSentinel + `","tool_arguments":"` + canonicalProjectionSensitiveSentinel + `","tool_result":"` + canonicalProjectionSensitiveSentinel + `","provider_payload":"` + canonicalProjectionSensitiveSentinel + `","raw_provider":"` + canonicalProjectionSensitiveSentinel + `","credential":"` + canonicalProjectionSensitiveSentinel + `"}`,
