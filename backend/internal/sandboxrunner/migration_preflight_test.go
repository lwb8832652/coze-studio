...
func TestCheckRequiredMigrationAcceptsOneFullyAppliedRevision(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(requiredMigrationQuery)).
		...
		WillReturnRows(sqlmock.NewRows([]string{"applied", "total", "error"}).AddRow(4, 4, nil))
	mock.ExpectRollback()
	mock.ExpectClose()
...
func TestCheckRequiredMigrationFailsClosedForNonAppliedRevisionStates(t *testing.T) {
	tests := map[string]*sqlmock.Rows{
		"missing":         sqlmock.NewRows([]string{"applied", "total", "error"}),
		"zero statements": sqlmock.NewRows([]string{"applied", "total", "error"}).AddRow(0, 0, nil),
		"partial":         sqlmock.NewRows([]string{"applied", "total", "error"}).AddRow(3, 4, nil),
		"failed":          sqlmock.NewRows([]string{"applied", "total", "error"}).AddRow(4, 4, "migration failed"),
		"duplicate": sqlmock.NewRows([]string{"applied", "total", "error"}).
			AddRow(4, 4, nil).
			AddRow(4, 4, nil),
	}
...
func TestCheckRequiredMigrationAcceptsEmptyAtlasError(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(requiredMigrationQuery)).
		WithArgs(requiredMigrationVersion).
		WillReturnRows(sqlmock.NewRows([]string{"applied", "total", "error"}).AddRow(4, 4, ""))
	mock.ExpectRollback()
	mock.ExpectClose()
