...
| P1-C 文档产物 MIME 样本库 | 进行中 | Markdown/TXT/CSV/PDF/image/HTML-SVG blocked/error/delete-restore cases each have UI evidence, API summaries, and unit/API tests. | 2026-07-01 automated source/test coverage map recorded in `notes/P1-C-artifact-mime-sample-map.md`; helper, task-detail, API handler, and application scan-policy tests passed. 2026-07-01 fixture task `7657556998191316992` now covers Markdown/TXT/JSON/CSV/PDF/PNG/HTML/SVG cards; Go ADK `write_file` supports binary `content_base64`; Markdown/PNG/HTML-SVG browser screenshots are recorded. Bounded API summaries and lifecycle evidence remain open. No raw object URI, signed URL, provider payload, or scanner raw body in evidence. |
...
| P1-C-003 UI screenshots per MIME family | 进行中 | Browser screenshots recorded for full card placement, Markdown preview, PNG preview, and HTML/SVG download-only active-content safety. Remaining: TXT/JSON/CSV/PDF side-preview screenshots and copy/download affordance checks. Evidence: `notes/P1-C-artifact-mime-sample-map.md`; screenshots under `screenshots/coze/P1-C-coze-*-7657556998191316992.png`. |
...
- Capture the remaining TXT/JSON/CSV/PDF side-preview screenshots and
  copy/download affordance checks from fixture task `7657556998191316992`.
