...
本设计只调整 `dev` 单实例部署拓扑和向量存储的未配置语义，不把完整本地开发栈
搬到服务器。
...
```text
Baota Nginx -> 127.0.0.1:8888 -> coze-web -> coze-server
                                              |
                                              +-> nsqd:4150
...
`coze-web` 仍只绑定宿主机回环地址。`coze-server` 和 `nsqd` 只加入 `coze-dev`
bridge 网络。NSQ 的 HTTP 端口 `4151` 仅用于容器内健康检查，不提供宿主机映射。
...
- 三个服务日志轮转配置一致；
- 原有 Web 回环端口、应用健康检查和只读 `app.env` 挂载合同不回归。
...
真实远程 MySQL、Redis、Elasticsearch、对象存储和宝塔服务器不参与本地自动测试。

## 10. 文档与长期事实

实现时同步更新：

- `deploy/dev/README.md`：运行拓扑、`app.env` 合同、安装、首次启动、排障和数据卷；
...
- Compose 合同测试、相关 Go 测试和文档检查全部通过。
