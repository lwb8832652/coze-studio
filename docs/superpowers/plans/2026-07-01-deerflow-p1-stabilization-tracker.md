...
| P1-I-004 Async memory update contract | 已完成 | Implemented DeerFlow-style terminal-only memory queueing in `backend/application/agentthread/adk_transcript.go`: `summary_input` no longer queues memory; terminal runs build a sanitized memory input from real user messages plus final assistant responses only; hidden dynamic reminders, Eino summarization control messages, tool calls/results, and `<uploaded_files>` blocks are excluded; upload-only turns skip the immediate assistant response; correction/reinforcement signals are captured in safe snapshot metadata. 2026-07-01: `ModelMemoryExtractor` now prompts/parses DeerFlow `user/history/newFacts` output, receives a safe current-memory JSON projection, and maps section summaries to safe `deerflow_section` memory metadata while preserving legacy `facts` compatibility. Tests: `go test ./application/agentthread -run 'TestApplicationProcessMemoryFlushJobs|TestModelMemoryExtractor|TestADKMemoryMiddleware|TestThreadMemoryProvider|TestADKTranscript' -count=1`; `go test ./application/agentthread -count=1`. |
| P1-I-005 Follow-up recall behavior | 进行中 | Backend recall query propagation completed: canonical follow-up run input already carries prior user/assistant turns plus latest user message; `ThreadMemoryProvider` now forwards the latest user query through application/domain `RecallMemoriesRequest` into repository memory search before local relevance sorting. Tests: `go test ./application/agentthread -run 'TestThreadMemoryProvider|TestADKMemoryMiddleware' -count=1`; `go test ./domain/agentthread/service -run 'TestRecallMemoriesNormalizesLimitAndUsesRunContext|TestManageMemoriesNormalizesAndDelegates' -count=1`. Remaining: browser/API evidence proving runtime recall/update works together on a real follow-up. |
...
| P1-J-006 Artifacts and present_files semantics | 进行中 | File creation, output presentation, artifact card position, side preview, download/copy, MIME handling, and final answer ordering match DeerFlow. 2026-07-01: artifact side-preview width now follows DeerFlow's 60/40 split instead of a fixed max-width drawer; evidence recorded in `docs/superpowers/evidence/2026-06-28-deerflow-task-detail-parity/notes/P1-J-006-artifact-split-width.md`. Remaining: paired screenshots and MIME/final-ordering acceptance. |
| P1-J-007 Streaming/stop/retry/follow-up | 进行中 | 2026-07-01: started source-first parity pass. This slice will verify DeerFlow prompt input stop-square, loading dots, retry/cancel affordances, and follow-up history preservation from source/runtime evidence before changing Coze. Required evidence: DeerFlow source anchors, Coze gap map, failing frontend test, passing targeted tests, and browser smoke. |
| P1-J-008 Quality acceptance suite | 进行中 | Same prompts run on DeerFlow and Coze: travel document, Mermaid diagram, search/answer, skill creation, MCP/weather, and multi-turn revision; compare visible output, steps, artifacts, tokens, and memory effects. 2026-07-01: task-detail ChainOfThought structure and spacing were tightened against DeerFlow `ChainOfThought` / `message-group.tsx`; frontend test now locks the DeerFlow-style chain container, collapsed last-step behavior, expanded step list, tool-specific labels, icons, and path pill. Browser DOM evidence recorded in `docs/superpowers/evidence/2026-06-28-deerflow-task-detail-parity/notes/P1-J-008-chain-of-thought-style.md`; paired screenshot evidence still needed before marking completed. |
