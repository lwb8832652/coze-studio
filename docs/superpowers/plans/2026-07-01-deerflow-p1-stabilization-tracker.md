...
| P1-C 文档产物 MIME 样本库 | 进行中 | Markdown/TXT/CSV/PDF/image/HTML-SVG blocked/error/delete-restore cases each have UI evidence, API summaries, and unit/API tests. | 2026-07-01 automated source/test coverage map recorded in `notes/P1-C-artifact-mime-sample-map.md`; helper, task-detail, API handler, and application scan-policy tests passed. 2026-07-01 fixture task `7657556998191316992` now covers Markdown/TXT/JSON/CSV/PDF/PNG/HTML/SVG cards; Go ADK `write_file` supports binary `content_base64`; Markdown/TXT/JSON/CSV/PNG/HTML-SVG browser screenshots are recorded. PDF right-side preview is implemented and unit-tested against DeerFlow iframe behavior; current fixture PDF was `scan_pending`, so service/task-detail tests now map bounded scan reasons to safe preview/download copy without weakening backend policy. The management panel review-release path changed the PDF to `clean` and mounted the PDF iframe with no object URI in `src`; screenshot records iframe container, not PDF page pixels. 2026-07-02 captured SVG artifact delete/restore UI lifecycle: remove confirmation, removed-list restore row, and restored active row; the pass found and fixed stale undo notice cleanup after deleted-list restore, with unit regression coverage and clean post-fix browser recapture. The same fixture records SVG scan review `blocked` -> `clean` with no unsafe visible payload markers. Copy/download affordance evidence is recorded; browser testing exposed a Clipboard API-only copy gap, now fixed with a textarea fallback for document preview and Mermaid source copy, covered by `task-clipboard.test.ts` and focused task-detail tests. Bounded API summaries are recorded from router/model/handler/test evidence; browser-authenticated fetch capture is N/A in the current read-only browser scope. Scan retry UI evidence is recorded: a single fixture job was temporarily moved to `failed`, the page showed `重试`, and the real retry action returned it to `pending` with bounded metadata. PDF pixel-render evidence and bounded error-state browser evidence remain open. No raw object URI, signed URL, provider payload, or scanner raw body in evidence. |
...
| P1-C-003 UI screenshots per MIME family | 进行中 | Browser screenshots recorded for full card placement, Markdown preview, TXT/JSON/CSV preview, PNG preview, HTML/SVG download-only active-content safety, PDF iframe mount after review release, and copy/download affordances. Scan-pending safe blocked-state copy is covered by service/task-detail tests. Remaining: PDF page-pixel render capture. Evidence: `notes/P1-C-artifact-mime-sample-map.md`; screenshots under `screenshots/coze/P1-C-coze-*-7657556998191316992.png`. |
...
- Continue artifact lifecycle evidence from fixture task `7657556998191316992`:
  bounded error states.
- Capture PDF page-pixel evidence if the local browser/PDF plugin can render
  the iframe contents; otherwise record the blocker separately from the already
  verified iframe mount and safe signed-preview URL behavior.
