...
output-budget, event, or audit policy. `duckduckgo` and `googlesearch` remain
backend candidates behind `ADKWebSearchBackend`; Google also requires Coze
secret storage and masked configuration before production use.

## Run Config

Web tools are disabled by default. They are enabled only through explicit run
configuration:
...
- requires an injected `ADKWebSearchBackend`;
...
- Add a DuckDuckGo backend only if network policy and retry behavior are
