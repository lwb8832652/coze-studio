...
| 消息/思考/Markdown | user/assistant turn, inline reasoning, Markdown/Mermaid | task messages + answer/result markdown | 已完成 | inline thinking 保留；assistant 回复复制/本地反馈操作已在 P2-A 补齐；Mermaid 错误图转 P2。 |
...
| TD-MSG-006 | 复制和反馈 | assistant turn 有复制/反馈操作 | Coze canonical/legacy assistant answer 均显示复制、赞、踩操作；复制只复制可见正文，本轮反馈为本地 UI 状态，持久化反馈 API 归 P2-B | unit DOM assertion；browser screenshot 可在后续 P2-A 视觉批次补 | feedback API persistence N/A for this slice | `task-assistant-message-actions.tsx`, `detail.tsx`, `task-result-section.tsx`, `task-detail.test.tsx` | 已完成 |
...
| TD-MD-003 | Mermaid 错误态 | 错误图不炸页面，可显示源码/错误提示 | Coze Mermaid 失败时进入 `data-status="error"`，显示错误提示和源码；错误态保留 `复制 Mermaid 源码`，不显示无效 SVG 下载 | unit DOM assertion；browser screenshot 可在后续 P2-A 视觉批次补 | message body sample；no backend contract for local rendering | `task-markdown-content.tsx`, `task-detail.test.tsx` | 已完成 |
...
| TD-FLOW-003 | 子智能体事件 | subagent 状态清楚 | Coze 子智能体卡片只显示安全 metadata：name/status/elapsed/terminal/model/token/cost/retry/timeline bounded error；不展示 child prompt/tool payload | targeted DOM test；browser screenshot 可在后续 P2-A 视觉批次补 | child run/events/token response mocked in unit test | `task-subagent-runs-section.tsx`, `task-detail-subagents.ts`, `adk_subagent_tool_provider.go`, `task-detail.test.tsx` | 已完成 |
...
| TD-COMP-006 | 文件上传 | composer 支持附件并展示上传文件 | Coze 目前只有同位附件入口；真实 upload/list/delete/submit/runtime `<uploaded_files>` 绑定已拆为 P2-F，不在 P2-A 伪完成 | composer DOM assertion；upload API/browser evidence pending P2-F | no Coze upload contract yet | `workbench-composer-controls.tsx`; follow-up evidence `P2-A-TD-COMP-006-upload-scope.md` | 延后(P2-F) |
