...
## 当前处置（2026-07-30 owner override）

五项零使用审计的第 3、4 项仍然缺少可查询的 gateway/service access logs、
外部 SDK consumer registry 和 named-owner roster，因此状态仍是 `BLOCKED`，
不得表述为“已证明外部零流量”。

2026-07-30，本次变更的项目决策者在已知上述证据缺口的前提下，明确要求
不保留兼容尾巴，并授权物理删除 10 条本地 `/api/runs/**` 路由及其专属
adapter/model/test。该 owner override 接受“仍可能存在不可见外部调用方”的剩余
风险，只覆盖原 Task 14 的执行条件，不改写五项审计的原始结果。
...
- 本地合同比较范围：canonical `/api/workbench/threads/**`、TaskThread V1
...
使用”。原五项门禁在未获得 owner override 时会禁止 Task 14；本文首节记录了
2026-07-30 的明确风险接受与执行授权。

## 历史 Gate A Candidate Source Status

Gate A 候选工作树当时重新执行 production string scan 后，状态为：
...
因此 Gate A 的 UI migration 当时可以通过，但第 3、4 项没有被后续浏览器流量
替代或补齐；按原门禁，Task 14 当时保持 `BLOCKED / RETAIN`。该历史决定后来
被本文首节的 owner override 覆盖。
...
当时结论：**BLOCKED / RETAIN**。第 3、4 项仍为 `BLOCKED`，因此 Task 14 没有增加负向
...
- 决定日期：`2026-07-30`。
- 决定类型：`OWNER_OVERRIDE / RETIRE`。
- 已知风险：第 3、4 项仍为 `BLOCKED`，外部业务调用方无法用现有证据排除。
- 授权范围：删除 10 条本地 `/api/runs/**` 路由、stateless handler/binding、
  专属测试和无其他调用方的手写 LangGraph model。
- 不在授权范围：canonical `/api/workbench/threads/**`、Scheduled Task、外部
  DeerFlow client 以及共享 application/domain/repository/MySQL/Eino ADK 主链。

实施必须同时保持 canonical 47 条路由、Scheduled Task 11 条路由和当前业务
语义，并对 10 条旧路由建立精确不可达断言。canonical SSE 需要的公开事件名与
payload shape 迁入 canonical-owned protocol 文件，不保留旧 HTTP adapter。
...
- 任何后续类似退役默认必须先补齐第 3、4 项固定窗口证据，并由对应
  系统 reviewer 复核；只有记录已知风险的明确项目 owner override 可以改变执行
  条件。无论是否 override，都不得沿用本地零调用结论作为外部零使用证明。
