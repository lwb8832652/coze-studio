...
| P1-B 任务详情视觉补证 | 已完成 | Task detail screenshots/API summaries cover layout, execution steps, inline thinking, Mermaid, Artifacts, export, token modes, memory entry, loading/error states, and narrow/mobile risk cases. | 2026-07-02 `TD-LAYOUT-002` is closed: task-detail now scopes narrow responsive overrides to the mounted page, 768px/390px browser evidence shows `horizontalOverflow=false`, and screenshots are recorded in `screenshots/coze/P1-B-TD-LAYOUT-002-coze-768-task-detail.png` and `screenshots/coze/P1-B-TD-LAYOUT-002-coze-390-task-detail.png`. Existing desktop evidence covers the main task-detail modules. Smaller non-blocking items moved to P2 hardening: assistant feedback affordance, real invalid Mermaid screenshot, subagent deep card sample, upload-path browser evidence, and active streaming token evidence. |
...
Continue with P1-D LangGraph compatibility suite:

- Build black-box coverage for thread/run/history/state/stream/cancel/join
  behavior beyond the DeerFlow-visible P0 paths.
